...
public class AuthenticateFilter(ICommandHandler<AuthenticateCommand, Result<SessionUser>> commandHandler, Config config)
    : IEndpointFilter
{
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        if (!context.HttpContext.Request.Cookies.TryGetValue(sessionCookieName,
                out var sessionId) || !Guid.TryParse(sessionId, out var guid))
        ...
        if (result.IsFailure)
        {
            CookieOptions cookieOptions =
                new() { Expires = DateTimeOffset.UtcNow.AddDays(-1) };
            context.HttpContext.Response.Cookies.Append(sessionCookieName, "",
                cookieOptions);
