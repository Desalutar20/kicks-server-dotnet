                        ...
                        ILoggerFactory loggerFactory) =>
                    {
                        ...
                        if (!Enum.TryParse<OAuthProvider>(provider, true, out var parsedProvider))
                        {
                            return ErrorHandler.Handle(
                                Error.Validation("provider", ["Invalid oauth provider"]), logger);
                        }
                        ...
                        {
                            Expires = DateTimeOffset.UtcNow.AddMinutes(config.Application.OAuthStateTtlMinutes),
                            HttpOnly = true,
                        ...
                        ctx.Response.Cookies.Append(config.Application.OAuthStateCookieName,
                            result.Value.Item2.ToString(), cookieOptions);

                        return Results.Redirect(result.Value.Item1.ToString());

                        // return Results.Ok(new ApiResponse<Uri>(result.Value.Item1));
                    })
