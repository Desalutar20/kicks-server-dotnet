...
{
    Task StoreVerificationTokenAsync(
        UserId userId,
        NonEmptyString token,
        TimeSpan ttl,
        CancellationToken ct = default
    );
    Task<UserId?> GetUserIdByVerificationTokenAsync(
        NonEmptyString token,
        CancellationToken ct = default
    );
    ...
    Task<UserId?> GetUserIdByPasswordResetTokenAsync(
        NonEmptyString token,
        CancellationToken ct = default
    );
    ...
    Task<IReadOnlyList<(Guid SessionId, DateTime CreatedAt)>> GetAllSessionsAsync(
        UserId userId,
        CancellationToken ct = default
    );
