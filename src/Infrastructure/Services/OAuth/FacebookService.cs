...
internal sealed class FacebookService(HttpClient httpClient, Config config) : IOAuthClient
{
    ...
    public async Task<Result<OAuthUser>> GetUserAsync(NonEmptyString code, CancellationToken ct = default)
    {
        ...
        {
            var tokenRequestUri =
                new Uri("https://graph.facebook.com/v20.0/oauth/access_token")
            ...
            var token = JsonSerializer.Deserialize<FacebookAccessTokenResponse>(tokenJson)
            ...
            var userResponse = await httpClient.GetAsync(
                new Uri("https://graph.facebook.com/v20.0/me")
                    ...
                    .AddParameter("access_token", token.AccessToken), ct);

            var userJson = await userResponse.Content.ReadAsStringAsync(ct);
            var user = JsonSerializer.Deserialize<FacebookUserResponse>(userJson)
