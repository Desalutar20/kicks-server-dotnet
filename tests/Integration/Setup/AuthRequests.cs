...
{
    protected async Task<HttpResponseMessage> SignUp(SignUpRequest data,
        CancellationToken ct = default) =>
        await Request(data, HttpMethod.Post, "/api/v1/auth/sign-up", null, ct);

    protected async Task<HttpResponseMessage>
        SignIn(SignInRequest data, CancellationToken ct = default) =>
        await Request(data, HttpMethod.Post, "/api/v1/auth/sign-in", null, ct);

    protected async Task<HttpResponseMessage>
        VerifyAccount(VerifyAccountRequest data, CancellationToken ct = default) =>
        await Request(data, HttpMethod.Post, "/api/v1/auth/verify-account", null, ct);


    protected async Task<HttpResponseMessage>
        ForgotPassword(ForgotPasswordRequest data, CancellationToken ct = default) =>
        await Request(data, HttpMethod.Post, "/api/v1/auth/forgot-password", null, ct);

    protected async Task<HttpResponseMessage>
        ResetPassword(ResetPasswordRequest data, CancellationToken ct = default) =>
        await Request(data, HttpMethod.Post, "/api/v1/auth/reset-password", null, ct);

    protected async Task<HttpResponseMessage>
        GetProfile(string? cookie, CancellationToken ct = default) =>
        await Request("/api/v1/auth/profile", cookie, ct);


    protected async Task
        CreateAndVerify(SignUpRequest data, CancellationToken ct = default)
    {
        ...
        var accountVerificationResponse =
            await VerifyAccount(new VerifyAccountRequest(token!, data.Email), ct);
        accountVerificationResponse.StatusCode.Should().Be(HttpStatusCode.OK);
    }

    protected async Task<string>
        CreateAndSignIn(SignUpRequest data, CancellationToken ct = default)
    {
        await CreateAndVerify(data, ct);

        var signInResponse = await SignIn(new SignInRequest(data.Email, data.Password),
        ...
        var sessionCookie = values!.FirstOrDefault(c => c.StartsWith(_config.Application.SessionCookieName));
