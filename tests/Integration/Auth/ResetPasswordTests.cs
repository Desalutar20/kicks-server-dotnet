...
public class ResetPasswordTests(ApiFactory factory) : TestApp(factory)
{
    ...
    public async ValueTask Should_ReturnOk_When_RequestIsValid()
    {
        ...
        var forgotPasswordResponse = await ForgotPassword(new ForgotPasswordRequest(request.Email), ct);
        ...
        var userBeforeUpdate = await GetUserFromDbByEmail(Email.Create(request.Email).Value);
        userBeforeUpdate.Should().NotBeNull();

        var response =
            await ResetPassword(new ResetPasswordRequest(token, request.Email, TestData.Password(Password.MinLength)),
                ct);
    ...
    public async ValueTask Should_ReturnBadRequest_When_EmailIsDifferent()
    {
        ...
        var resetPasswordResponse =
            await ResetPassword(
                new ResetPasswordRequest(token!, TestData.Email(), TestData.Password(Password.MinLength)),
                ct);
    ...
    [
        ("token", new ResetPasswordRequest("", TestData.Email(), TestData.Password(Password.MinLength))),
        ("email", new ResetPasswordRequest("token", "", TestData.Password(Password.MinLength))),
        ("email", new ResetPasswordRequest("token", "invalid email", TestData.Password(Password.MinLength))),
        ("newPassword", new ResetPasswordRequest("token", TestData.Email(), "")),
        ("newPassword", new ResetPasswordRequest("token", TestData.Email(), TestData.Password(Password.MinLength - 1))),
        ("newPassword", new ResetPasswordRequest("token", TestData.Email(), TestData.Password(Password.MaxLength + 1)))
    ];
}