...
public sealed class DeleteUserTests(ApiFactory factory) : TestApp(factory)
{
    [Fact]
    ...
    public async ValueTask Should_ReturnBadRequest_When_UserNotFound()
    {
        ...
        var response = await DeleteUser(new UserId(Guid.NewGuid()), sessionCookie, ct);
    ...
    public async ValueTask Should_ReturnUnauthorized_When_UserNotSignedIn()
    {
        ...
        var response = await DeleteUser(new UserId(Guid.Empty), null, ct);
    ...
    public async ValueTask Should_ReturnForbidden_When_UserIsNotAdmin()
    {
        ...
        var response = await DeleteUser(new UserId(Guid.Empty), sessionCookie, ct);
